...
def dashboard(request):
    ...
    guests_without_diets = attending_guests.filter(
        Q(diet__isnull=True) | Q(diet='')
    ).order_by(
        'party__category', 'first_name'
    )
    diet_breakdown = attending_guests.exclude(diet=None).values('diet').annotate(count=Count('*'))
    ...
    return render(request, 'guests/dashboard.html', context={
        ...
        'guests_without_diets': guests_without_diets,
...
InviteResponse = namedtuple('InviteResponse', ['guest_pk', 'is_attending', 'diet'])
